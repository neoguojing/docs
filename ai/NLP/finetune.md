...
## LoRA 低秩适应大型语言模型
- 原理：模型是过参数化的，它们有更小的内在维度，模型主要依赖于这个低的内在维度（low intrinsic dimension）去做任务适配。
...
- 梯度：参数量1倍
- 优化器的状态：参数量的2倍
- - 用 AdamW 优化器，8字节存储参数：需要存储参数、梯度和二阶梯度；
- - SGD 优化器，需要 1倍 参数量；
- - AdaFactor：4字节存储参数
- - 位和字节优化器如8bit的Adam，需要1字节存储
- 显存预估为：餐数量*4 7B*4*4 = 112g
#### lora训练
- 原本需要微调1024×512的参数，用了LoRA之后，如果选择的 Rank=8，则只需要微调这么多参数：1024×8+512×8
#### qlora训练
#### gptq训练
