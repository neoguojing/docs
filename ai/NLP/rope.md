GitHub 的 Markdown 引擎（cmark-gfm + KaTeX）与常规 Markdown 渲染器不同，它存在以下**致命的解析冲突**：

1. **矩阵换行 `\\` 被 Markdown 吞噬**：Markdown 渲染器先于 KaTeX 执行，会将 `\\` 转义吃成单斜杠 `\`，导致矩阵无法换行而报错崩溃。**（最彻底的解决方案：在矩阵内用原生 TeX 行分隔符 `\cr` 替代 `\\`）**。
2. **表格里的范数符号 `|` 炸毁表格**：在 Markdown 表格中，`\|x\|` 里的竖线会被误认为是“单元格分隔符”，直接破坏整张表格。**（解决方案：表格及公式中的双竖线范数一律改用 `\Vert x \Vert`）**。
3. **下划线 `_` 触发斜体语法**：如 `\cos\_vec` 或多个带下标的变量（如 `$m\theta_0$` 到 `$m\theta_1$`），会被 Markdown 贪婪匹配解析成 `<em>` 标签。**（解决方案：代码变量使用行内代码反引号包裹，数学公式规范收敛）**。
4. **行级公式前后空行**：每个 `$$` 块上方和下方**必须至少保留一个纯空行**。

以下是针对 GitHub 严格适配、修复上述所有冲突的完整代码：
