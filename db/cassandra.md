...
### 1.1 为什么 Cassandra 极快且易于扩展？
* **LSM 树极致的顺序写入 (Log-Structured Merge-Tree)**
  传统关系型数据库（如 B+ 树）在写入时需要进行大量随机 I/O 来更新树结构。Cassandra 的写入是纯粹的**顺序追加**：
  1. 数据首先顺序写入磁盘的 **Commit Log**（保证系统崩溃时的数据恢复）。
  2. 同步写入内存中的 **Memtable**（此时即向客户端返回“写入成功”，耗时极短）。
  3. Memtable 写满后，作为不可变的 **SSTable** 顺序刷入磁盘。这种将“随机写”转换为“顺序写”的设计，直接榨干了磁盘的吞吐上限。
* **无中心化的分布式架构 (Masterless)**
  采用基于一致性哈希的 P2P 环形架构，集群中**没有主从节点之分**。客户端请求可以路由到任何一个节点，由该节点作为协调者并发处理，彻底消除了单点故障（SPOF）和主节点的性能瓶颈。
* **近乎完美的线性水平扩展 (Linear Scalability)**
  系统吞吐量与节点数量呈严格的正比关系。当存储容量或计算能力遇到瓶颈时，只需加入新节点，数据便会自动在后台重新分布（Rebalancing），无需停机、无需手动分库分表。
* **读优化的 Bloom Filter 与索引机制**
  LSM 树虽然牺牲了部分读取性能，但 Cassandra 利用内存中的 **Bloom Filter** 快速拦截“绝对不存在的查询”，避免无效的磁盘 I/O。结合 Partition Key 索引，能够在海量 SSTable 中精准过滤出目标数据块。

### 1.2 适合场景
作为典型的 **AP 架构**（优先保证高可用与分区容错），Cassandra 最适合“写密集型”或跨地域部署的业务：
* **海量数据、高并发、高吞吐**的业务。
* **KV 查询、日志、时间序列、用户行为数据、消息 / 事件数据**。
* **IoT (物联网)**：传感器、智能设备会产生海量且连绵不断的流数据。Cassandra 极高的并发写入能力和基于 Clustering Column 的自动时间序排列完美契合。
* **全球多机房部署 (Multi-DC)**：原生支持跨数据中心、跨地域的 Active-Active（多活）复制。异地机房均可提供毫秒级的就近读写，且能无缝容忍单机房整体宕机。
* **对高可用要求高的系统**。

### 1.3 不适合场景（核心避坑）
Cassandra 采用**查询驱动建模 (Query-Driven Modeling)**，必须在建表前完全确定查询模式，违背此原则将导致严重的性能灾难。
* **复杂 JOIN**：不支持多表 JOIN、复杂子查询和跨节点的动态聚合运算。若强制执行，会引发全表扫描及网络风暴。建议改用 ClickHouse, Elasticsearch。
* **强依赖 ACID 事务**：仅支持单行（Partition）级别的原子性，不支持跨行、跨表的多阶段回滚事务。转账等金融核心链路存在不一致风险。建议改用 MySQL, TiDB。
* **强依赖关系约束的业务**：数据库层面没有外键，不提供关系校验引擎。如果在应用层手动实现关联查询，会产生极其高昂的延迟。建议改用 PostgreSQL, Neo4j。
* **复杂报表查询、大量跨 Partition 操作、查询模式高度不确定**。

---

## 第二部分：查询驱动建模 (Query-Driven Modeling)

关系型数据库的建模核心是“消除冗余”，而 Cassandra 彻底反其道而行之：**空间换时间，消除关联（JOIN）**。查询驱动建模要求你在建表前，必须绝对清晰地知道业务最终会发送什么样的查询请求。

### 2.1 三大核心设计原则
* **一张表只为一个查询服务 (One Query = One Table)**：既然不支持跨表 JOIN，如果你需要同时展示“用户信息”和“该用户最近的订单”，就必须在写入时将这两部分数据捏合，写入到同一张 `orders_by_user` 表中。
* **拥抱反范式与数据冗余**：磁盘极其廉价，而跨节点网络 I/O 极其昂贵。如果业务既需要“通过手机号查用户”，又需要“通过邮箱查用户”，标准的做法是建两张完全独立但数据高度重合的表（`users_by_phone` 和 `users_by_email`），各自服务对应的查询。
* **让一次查询只命中一个分区**：完美状态下，一次查询请求应该只访问集群中的一个节点。如果你的查询需要跨越成百上千个分区去捞取数据（Scatter-Gather），Cassandra 的性能会呈现断崖式下跌。

### 2.2 解构灵魂：主键（Primary Key）设计
在 Cassandra 中，主键不仅仅是数据的唯一标识，它直接决定了**数据存在哪台机器上**以及**数据在硬盘上如何排序**。主键由两部分组成：`PRIMARY KEY (Partition Key, Clustering Column)`。

* **Partition Key (分区键)**：决定数据分布在集群的哪个节点上。它是哈希环上的一个锚点。同一个 Partition Key 的所有数据必定物理存储在同一节点上。
  * *设计雷区*：数据倾斜。比如用“国家”做分区键，导致海量数据挤爆单台机器。
* **Clustering Column (集群列)**：决定数据在单个分区内部的物理排序方式（磁盘上的连续存放顺序，正序或倒序）。
  * *设计雷区*：必须符合业务范围查询的需求，否则无法执行 `<`、`>` 或是 `ORDER BY`。

### 2.3 实战推演：物联网（IoT）温度回溯
假设业务需求：“查询 1 号传感器在过去 24 小时内的所有温度记录，按时间从新到旧排列。”

在 Cassandra 中，表结构必须完全贴合查询：
* **表名约定**：`temperature_by_sensor` （明确告知这是以传感器维度查询的表）。
* **分区键选型**：`sensor_id`。这样 1 号传感器的所有数据都集中在一台节点上，查询时不会引发全网风暴。
* **集群列选型**：`recorded_at DESC`。确保每次写入时，最新的温度数据都紧紧排在分区最顶部，读取时直接按顺序扫盘，延迟极低。
