...
#### 2.2.2 微观寻址结构 (Index -> Granules -> Blocks)
...
### 2.6 数据瘦身术：编码 (Encoding) 与 压缩 (Compression)

ClickHouse 采用“先编码，后压缩”的两步走策略，将磁盘 I/O 压榨到极致。

#### 2.6.1 第一步：数据编码 (Encoding) 举例
...
#### 2.6.2 第二步：通用压缩 (Compression) 过程举例
...
#### 3.5.2 分布式表 Join 的坑：GLOBAL JOIN
...
### 4.2 稀疏主键索引 (Sparse Primary Key Index) 与 Granule
...
### 4.3 Data Skipping Index (数据跳跃索引)
...
### 7.1 数据如何落盘？（核心机制：不可变 Part）

ClickHouse 的数据落盘机制并不像传统数据库那样零散写入，而是基于**不可变（Immutable）的数据块（Part）**进行批量操作。

*   **写入过程**：每次 `INSERT` 批次请求到达时，ClickHouse 会在内存中对数据进行排序（依据 Order By 键），然后在磁盘上生成一个临时目录，写入列数据（`.bin`）、索引（`.idx`）和标记（`.mrk`）文件。
*   **原子生效**：数据完全写入临时目录后，通过文件系统的 `rename` 操作将其重命名为正式的 Part 目录。这一步是原子的，重命名完成后，数据即对查询可见。
*   **后台合并（Merge）**：为了防止小文件爆炸和提升查询效率，后台会周期性地启动 Merge 线程，将多个小的 Part 读取到内存中，合并、去重并生成一个全新的大 Part，随后旧的 Part 被标记为非活跃并最终被物理删除。

### 7.2 Crash 后怎么办？（基于目录的快速恢复）

ClickHouse 没有传统 OLTP 复杂的 Buffer Pool 脏页刷盘机制，其恢复过程非常轻量。

*   **已提交数据**：因为每次成功的 `INSERT` 都已经落盘成完整的 Part 目录，进程 Crash 并重启后，ClickHouse 只需扫描表目录下的所有合法 Part 文件夹，加载其元数据（索引和标记）到内存中，数据即可恢复。
*   **未提交/半写入数据**：如果在 `INSERT` 过程中 Crash（临时目录未完成 `rename`），重启时 ClickHouse 会识别出这些处于中间状态的 `tmp_` 目录并将其清理掉。这就要求**客户端（Client）必须具备重试机制**，重新发送未收到成功 Ack 的数据批次。

### 7.3 WAL 与写入一致性（非典型事务模型）

面试中经常被问到 ClickHouse 的一致性模型，必须明确它与 MySQL/PostgreSQL 的区别：

| 机制维度 | 传统 OLTP (如 MySQL) | ClickHouse (MergeTree) |
| :--- | :--- | :--- |
| **持久化路径** | 内存修改 -> 记 WAL (Redo Log) -> 异步刷脏页 | 内存排序 -> 直接生成完整磁盘文件 (Part) -> 原子重命名可见 |
| **小批量写入** | 极度友好，WAL 保证效率 | 极度不友好，会导致底层生成海量小文件，压垮 Zookeeper 和文件系统 |
| **事务支持** | 强 ACID | 默认不支持标准事务（仅保证单批次 INSERT 的原子性） |

*注意：虽然 ClickHouse 在新版本引入了基于内存的 WAL 特性（`in_memory_parts_enable_wal`）来缓解小批量写入问题，但其底层架构的灵魂依然是**大批次写入 + Part 文件生成**。*

### 7.4 副本恢复（依赖 ZooKeeper / ClickHouse Keeper）

`ReplicatedMergeTree` 表引擎负责副本的高可用。当节点 A 宕机，而节点 B 存活并继续接收写入时：

*   **元数据同步**：每次写入节点 B，B 成功生成 Part 后，会将该 Part 的元数据记录到 ZooKeeper 的 Log 节点中。
*   **自动拉取机制**：当节点 A 恢复启动时，会与 ZooKeeper 进行通信，比对自身的本地 Part 列表与 ZooKeeper 中的 Log。
*   **点对点传输**：节点 A 发现自己缺失了某些 Part，便会生成 Fetch 任务，通过 HTTP 协议直接从节点 B 下载缺失的物理 Part 文件到本地，完成数据对齐。

### 7.5 备份（Backup ≠ 副本）

永远不要把多副本等同于备份。副本同步的是状态，而备份保存的是历史快照。

*   **副本（Replica）防硬件故障**：如果主板烧毁、断网，副本可以秒级接管。但如果 DBA 执行了 `DROP TABLE` 或者程序引入了脏数据，这种破坏性操作会立刻同步给所有副本，导致数据全毁。
*   **备份（Backup）防逻辑灾难**：需要定期将底层 Part 文件硬链接打包，上传到廉价的远程存储（如 S3 对象存储、HDFS）。
*   **常用备份方案**：推荐使用 ClickHouse 原生的 `BACKUP / RESTORE` SQL 指令（22.8+ 版本），或者业界流行的第三方开源工具 `clickhouse-backup`，实现每日全量/增量快照并推送到异地机房。
